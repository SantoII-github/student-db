...
echo "$($PSQL "SELECT course FROM courses FULL JOIN majors_courses USING(course_id) FULL JOIN students USING(major_id) WHERE student_id IS NOT NULL GROUP BY course HAVING COUNT(*) = 1 ORDER BY course")"